...
public class ReaderRenderer {
	...
	static class CodeAndResult {
		...
		private CodeAndResult(CodeBlock code, CodeBlock result) {
	...
	private CodeBlock buildReadFieldCase(FieldView field) {
		var type = field.thriftType();
		var codeAndResult = buildReadField(type, 0);
	...
	private CodeAndResult buildReadList(ThriftListType type, Integer level) {
		var elementsCodeAndResult = buildReadField(type.subtype(), level + 1);
		...
		var result = CodeBlock.of("collection$L", level);
	...
	private TypeSpec buildBuilder(View view) {
		var validationMethod = buildValidationMethod(view);
